...
app.post('/api/settings/ai', asyncHandler(async (req, res) => {
    ...
    await prisma.user.update({ 
        ...
        data: { aiProvider: provider, aiApiKey: apiKey, slideStructure: slideStructure } 
    });
    res.json({ success: true });
}));
...
app.post('/api/ai/slides', asyncHandler(async (req, res) => {
    ...
    const systemPrompt = `You are a master teacher generating a presentation slide deck. The curriculum framework is: ${curriculum}. Differentiate instructions for ${keyStage} using the scale: ${differentiation}.
...
The array MUST contain slides corresponding to this exact sequence/structure provided by the user:
${slideHeadings}`;
    ...
    const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
            model: user.aiProvider === 'openrouter' ? 'anthropic/claude-3.5-sonnet' : 'gpt-4o',
...
app.post('/api/ai/toolkit', asyncHandler(async (req, res) => {
    ...
    else if (tool === 'observation') systemPrompt += ` Write constructive, professional, formal lesson observation feedback based on these notes. Detail strengths and areas for development.`;
    ...
    const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
            model: user.aiProvider === 'openrouter' ? 'anthropic/claude-3.5-sonnet' : 'gpt-4o',
            messages: [{ role: "system", content: systemPrompt }, { role: "user", content: `Context/Topic: ${topic}` }]
        })
    });
