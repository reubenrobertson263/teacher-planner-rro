window.tasksController = {
    ...
    renderBoard() {
        ...
        board.innerHTML = this.tasks.map(t => `
            <div class="settings-card" style="position:relative; cursor:pointer;" onclick="tasksController.editTask('${t.id}')">
                ...
                <div style="font-size:0.9em; color:var(--text-muted); line-height:1.4;">${t.status === 'DONE' ? '<span style="color:#10b981; font-weight:700;">✓ Completed</span>' : 'Active Task'}</div>
            </div>
        `).join('');
    },
    ...
    openNoteModal() {
        ...
        if(modal) {
            ...
            window.app.showToast("Error: Modal HTML not found.");
        }
    },
